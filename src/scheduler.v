...
module scheduler #(
    ...
    assign fifo_data_in = CTRL_SCHED_EVENT_IN
                        ? {CTRL_SCHED_VIRTS, 1'b0, CTRL_SCHED_ADDR}     // Source: AERIN (External)     -> Tag:0
                        : {4'b0            , 1'b1, CTRL_NEURMEM_ADDR};  // Source: Recursive (Internal) -> Tag:1 
    ...
    //assign push_req_n = ~((~SPI_OPEN_LOOP_sync & NEUR_EVENT_OUT) | CTRL_SCHED_EVENT_IN); // To deactivate SPI interface we need to set SPI_OPEN_LOOP = 0 for Neural Event
    assign push_req_n = ~((~1'b0 & NEUR_EVENT_OUT) | CTRL_SCHED_EVENT_IN);
